...
import BuyOrderDistribution from "@/model/buynow-distribution";
...
export async function POST(req) {
  try {
    ...
    const orders = await BuyNow.find({ date: today });
    ...
    const updatedOrders = await BuyNow.find({ date: today }).populate("rider_id", "name mobile");
    ...
    await BuyOrderDistribution.create({
      date: today,
      totalOrders,
      totalRiders,
      avgOrdersPerRider,
